...
def extract_text_fields(profile_data: dict) -> List[Tuple[str, str, str]]:
    ...
    if 'certifications' in profile_data:
        for i, cert in enumerate(profile_data.get('certifications', [])):
            ...
            if 'issuer' in cert:
                cert_text.append(f"issued by {cert['issuer']}")
            if 'date' in cert:
                cert_text.append(f"on {cert['date']}")
            if cert_text:
                text_fields.append(('certification', str(i), ', '.join(cert_text)))
    ...
    if 'education' in profile_data:
        for i, edu in enumerate(profile_data.get('education', [])):
            ...
            if 'institution' in edu:
                edu_text.append(f"at {edu['institution']}")
            if 'description' in edu:
                edu_text.append(edu['description'])
            if edu_text:
                text_fields.append(('education', str(i), ' '.join(edu_text)))
    
    for key in ['summary', 'bio', 'objective', 'interests', 'awards']:
