...
class ToolBox:
    ...
    async def _retrieve_context_tool(self, section_id: Optional[str] = None) -> str:
        ...
        # The retrieval service in the other modules uses /retrieve/full and /retrieve/section
        endpoint = f"{RETRIEVAL_SERVICE_URL.rstrip('/')}/{'retrieve/full' if not section_id else 'retrieve/section'}"
        payload = {"user_id": context["user_id"], "job_description": context["job_description"]}
        if section_id: payload["section_id"] = section_id
        try:
            response = await self.http_client.post(endpoint, json=payload, timeout=30.0)
    ...
    async def _generate_text_tool(self, section_id: Optional[str] = None, existing_text: Optional[str] = None) -> str:
        ...
        try:
            response = await self.http_client.post(endpoint, json=payload, timeout=90.0)
            response.raise_for_status()
    ...
    def _get_full_resume_text_tool(self) -> str:
        ...
        for section, content in resume_state.items():
            ...
            if isinstance(content, list):
                # Handle complex list items like experience objects
                for item in content:
    ...
    async def _score_resume_text_tool(self, resume_text: str) -> str:
        ...
        try:
            response = await self.http_client.post(endpoint, json=payload, timeout=45.0)
            response.raise_for_status()
            score_data = ScoreResponse(**response.json())
            # FIX: Use score_data.final_score and provide a more detailed result string
            return (
                f"Scoring Result: Final Score = {score_data.final_score:.2f} "
                f"(Semantic: {score_data.semantic_score:.2f}, Keyword: {score_data.keyword_score:.2f}), "
                f"Missing Keywords = {score_data.missing_keywords}"
            )
